...
echo "=========================================="
echo "Zigbee Matter Mnager Deployment Script"
echo "=========================================="
echo
...
if [ -f "zigbee-matter.service" ]; then
    # Update paths in service file
    sed -i "s|WorkingDirectory=.*|WorkingDirectory=$INSTALL_DIR|g" zigbee-matter.service
    sed -i "s|ExecStart=.*|ExecStart=$VENV_DIR/bin/python main.py|g" zigbee-matter.service
    sed -i "s|ReadWritePaths=.*|ReadWritePaths=$LOG_DIR $INSTALL_DIR|g" zigbee-matter.service

    cp zigbee-matter.service /etc/systemd/system/
    systemctl daemon-reload
    systemctl enable zigbee-matter
    echo "✓ Systemd service installed and enabled"
else
    echo "⚠ Warning: zigbee-matter.service file not found"
fi
...
if [ -f "zigbee-logrotate.conf" ]; then
    ...
    cp zigbee-logrotate.conf /etc/logrotate.d/zigbee-matter
    chmod 644 /etc/logrotate.d/zigbee-matter
    echo "✓ Logrotate configuration installed"

    # Test logrotate config
    echo "  Testing logrotate configuration..."
    if logrotate -d /etc/logrotate.d/zigbee-matter >/dev/null 2>&1; then
...
echo "- Stop service: sudo systemctl stop zigbee-manger"
