            ...
            if ep_id == EP_SYSTEM {
                match &frame {
                    ...
                    CpcFrame::Data { seq_num, .. } => {
                        // RCP is retransmitting stale I-frames from a previous
                        // session.  We MUST acknowledge them with correct RR
                        // N(R) — the RCP will not accept SABM/DISC until its
                        // retransmit queue is cleared.
                        //
                        // Use a dedicated ep0 EpLinkState (keyed as EP_SYSTEM)
                        // to track the sequence, even though ep0 doesn't have
                        // a registered sink.
                        let seq = *seq_num;
                        let rr_nr = {
                            let state = self.ep_state.entry(EP_SYSTEM).or_default();
                            state.record_received(seq);
                            state.expected_seq
                        };
                        let rr = Bytes::from(encode_rr(EP_SYSTEM, rr_nr));
...
mod tests {
    ...
    #[tokio::test]
    async fn ep0_stale_iframe_gets_rr() {
        // When the RCP retransmits stale I-frames on ep0 from a previous
        // session, the router must respond with RR to clear the retransmit
        // queue — otherwise the RCP ignores all subsequent SABM/DISC.
        let (mut router, inject, mut capture, _ep_rx, _ep_tx) =
            make_router_with_ep(EP_ZIGBEE);

        // Simulate RCP retransmitting stale ep0 I-frame with N(S)=0
        inject.send(make_wire(EP_SYSTEM, 0x00, b"stale_data")).await.unwrap();
        let chunk = router.serial.rx.recv().await.unwrap();
        router.handle_rx_chunk(chunk).await;

        // Router must send RR with N(R)=1
        let sent = capture.recv().await.unwrap();
        let (ep, ctrl) = wire_ep_ctrl(&sent);
        assert_eq!(ep, EP_SYSTEM, "RR must target ep0");
        // S-frame RR with N(R)=1: ctrl = 0x01 | (1 << 5) = 0x21
        assert_eq!(ctrl, 0x21, "must send RR N(R)=1 for stale ep0 I-frame");
    }

    #[tokio::test]
    async fn ep0_iframe_not_forwarded_to_any_sink() {
        // ep0 I-frames get RR but must NOT be forwarded to endpoint sinks
        let (mut router, inject, _capture, mut ep_rx, _ep_tx) =
            make_router_with_ep(EP_SYSTEM);

        inject.send(make_wire(EP_SYSTEM, 0x00, b"noise")).await.unwrap();
        let chunk = router.serial.rx.recv().await.unwrap();
        router.handle_rx_chunk(chunk).await;

        // Nothing should be forwarded to the endpoint sink
        assert!(ep_rx.try_recv().is_err(), "ep0 I-frame should not be forwarded");
    }
